...
def admin_read_posts(data: dict[str, Any] = Depends(get_current_user)):
    try:
        if data["role"] == 'admin':
            response = supabase.table("posts").select("*, user_to_role(user_name, role)").execute()
            print(response.data)
...
def club_read_post(id: str, data: dict[str, Any] = Depends(get_current_user)):
    try:
        if data["role"] == "admin":
            response = supabase.table("posts").select("*, user_to_role(user_name, role)").eq("post_id", id).execute()
            if not response.data:
